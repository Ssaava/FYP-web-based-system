...
interface QualityTrendsData { // For the potability trends chart
  label: string;
  potable: number;
  nonPotable: number;
}
...
export default function AnalyticsPage() {
  const [predictiveChartData, setPredictiveChartData] = useState<PredictiveChartDataPoint[]>([]);
  const [qualityTrendsData, setQualityTrendsData] = useState<QualityTrendsData[]>([]); // For forecasted potability
  ...
  const formatTimestampForXAxis = (timestamp: number): string => {
    ...
    return `${dateObj.toLocaleDateString(undefined, { month: 'numeric', day: 'numeric' })} ${dateObj.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit', hour12: false })}`;
  };
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        if (currentDataResponse.ok) {
          actualReadings = (await currentDataResponse.json()) as MlModelApiReading[];
        } else {
          const errorData = await currentDataResponse.json().catch(() => ({error: "Failed to parse current data error response"}));
          console.warn(`Failed to fetch current data: ${currentDataResponse.status}`, errorData.error || "");
          // Decide if this is a critical error or if we can proceed with only forecast
        }
        
        // Process Forecast Data
        const forecastResponseData: ForecastDataPayload | ApiError = await forecastDataResponse.json();
        if (!forecastDataResponse.ok || ('error' in forecastResponseData && forecastResponseData.error)) {
            const errorMessage = ('error' in forecastResponseData && forecastResponseData.error) ? forecastResponseData.error : `Forecast API request failed (${forecastDataResponse.status})`;
        ...
        const validForecastPayload = forecastResponseData as ForecastDataPayload;
        ...
        actualReadings.forEach(reading => {
          ...
          if (isNaN(timestampMs)) {
            console.warn("Invalid timestamp in actual reading:", reading.Timestamp);
            return;
          }
        ...
        const numForecastEntries = Object.keys(validForecastPayload.timestamps).length;
        ...
        for (let i = 0; i < numForecastEntries; i++) {
          ...
          if (existingEntry) { // Should not happen if timestamps are unique
        ...
        if (lastActualTimestamp > 0 && numForecastEntries > 0) {
            ...
            if (lastActualPoint && firstForecastTimestamp && lastActualTimestamp !== firstForecastTimestamp) {
                // If the first forecast point is not already the last actual point
                const firstForecastPointData = combinedDataMap.get(firstForecastTimestamp);
                if (firstForecastPointData) {
                     // Make the first forecast data point also carry the last actual values for forecast series
                    firstForecastPointData.ph_forecast = lastActualPoint.ph_actual ?? firstForecastPointData.ph_forecast;
                    firstForecastPointData.turbidity_forecast = lastActualPoint.turbidity_actual ?? firstForecastPointData.turbidity_forecast;
                    firstForecastPointData.conductivity_forecast = lastActualPoint.conductivity_actual ?? firstForecastPointData.conductivity_forecast;
                    firstForecastPointData.temperature_forecast = lastActualPoint.temperature_actual ?? firstForecastPointData.temperature_forecast;
                }
            } else if (lastActualPoint && firstForecastTimestamp && lastActualTimestamp === firstForecastTimestamp) {
        ...
        const finalPredictiveChartData = Array.from(combinedDataMap.values()).sort((a, b) => a.timestamp - b.timestamp);
        setPredictiveChartData(finalPredictiveChartData);


        // --- Process Quality Trends (uses only forecast potability) ---
        if ('predicted_potability' in validForecastPayload && validForecastPayload.predicted_potability) {
            let potableCount = 0;
        ...
        setError(err.message || "An unknown error occurred on the analytics page.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="flex-1 space-y-6 p-4 md:p-8 pt-6">
      <div className="flex items-center justify-between space-y-2">
        <div>
          ...
          <p className="text-muted-foreground">
            Historical data, current status, and future predictions of water quality parameters
          </p>
        </div>
      </div>
      ...
      {!isLoading && !error && predictiveChartData.length === 0 && qualityTrendsData.length === 0 && (
         <Card>
          <CardHeader>
          ...
          <CardContent>
            <p className="text-muted-foreground">
              No historical or forecast data is currently available to display.
            </p>
          </CardContent>
        </Card>
      )}

      {!isLoading && !error && (predictiveChartData.length > 0 || (qualityTrendsData.length > 0 && (qualityTrendsData[0].potable > 0 || qualityTrendsData[0].nonPotable > 0))) && (
        <>
          {predictiveChartData.length > 0 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Showing historical trends and a {forecastPeriods}-period forecast. Forecasted values are indicated by dotted lines.
                </CardDescription>
              </CardHeader>
          ...
          {qualityTrendsData.length > 0 && (qualityTrendsData[0].potable > 0 || qualityTrendsData[0].nonPotable > 0) && (
            <Card>
              <CardHeader>
                <CardTitle>Forecasted Potability Trend</CardTitle>
                <CardDescription>
                  Aggregated predicted potability for the next {forecastPeriods} periods.
                </CardDescription>
              </CardHeader>
              <CardContent>
