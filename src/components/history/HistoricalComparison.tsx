...
import { ProcessedReading, ComparisonParameterData } from "@/components/history/types"; // Adjust path
import { subDays, subWeeks, subMonths, subQuarters, subYears, startOfDay, endOfDay, startOfWeek, endOfWeek, startOfMonth, endOfMonth, startOfQuarter, endOfQuarter, startOfYear, endOfYear, isValid } from 'date-fns'; // Date manipulation

interface HistoricalComparisonProps {
  allReadings: ProcessedReading[];
}

const PARAMETERS_TO_COMPARE: Array<keyof Pick<ProcessedReading, 'ph' | 'temperature' | 'turbidity' | 'conductivity'>> = [
...
const calculateAverage = (readings: ProcessedReading[], parameter: keyof ProcessedReading, startDate: Date, endDate: Date): number | undefined => {
  const filteredReadings = readings.filter(r => {
    const readingDate = new Date(r.timestampMs);
    return readingDate >= startDate && readingDate <= endDate;
  });

  if (filteredReadings.length === 0) return undefined;

  const sum = filteredReadings.reduce((acc, curr) => acc + (Number(curr[parameter]) || 0), 0);
  return sum / filteredReadings.length;
};

export function HistoricalComparison({ allReadings }: HistoricalComparisonProps) {
  ...
  const comparisonChartData = useMemo((): ComparisonParameterData[] => {
    ...
    let baselinePeriodStart: Date | undefined, baselinePeriodEnd: Date | undefined;
    ...
    switch (comparisonType) {
      ...
      case "baseline":
        ...
        if (allReadings.length > 0) {
          const firstReadingDate = new Date(allReadings[0].timestampMs);
          if(isValid(firstReadingDate)){
            baselinePeriodStart = startOfDay(firstReadingDate);
            baselinePeriodEnd = endOfDay(subDays(startOfDay(addDays(firstReadingDate, 29)),0)); // end of 30th day
          }
        }
    ...
    if (!isValid(currentPeriodStart) || !isValid(previousPeriodStart) || !isValid(previousPeriodEnd)) {
        console.error("Invalid date calculation for periods.");
        return [];
    }


    const data: ComparisonParameterData[] = PARAMETERS_TO_COMPARE.map(paramKey => {
      const paramName = PARAMETER_NAMES[paramKey] || paramKey;
      const currentAvg = calculateAverage(allReadings, paramKey, currentPeriodStart, currentPeriodEnd);
      const previousAvg = calculateAverage(allReadings, paramKey, previousPeriodStart, previousPeriodEnd);
      let baselineAvg: number | undefined;

      if (comparisonType === "baseline" && baselinePeriodStart && baselinePeriodEnd && isValid(baselinePeriodStart) && isValid(baselinePeriodEnd)) {
        baselineAvg = calculateAverage(allReadings, paramKey, baselinePeriodStart, baselinePeriodEnd);
      }

      return {
        parameter: paramName,
        current: currentAvg !== undefined ? parseFloat(currentAvg.toFixed(2)) : undefined,
        previous: previousAvg !== undefined ? parseFloat(previousAvg.toFixed(2)) : undefined,
        baseline: baselineAvg !== undefined ? parseFloat(baselineAvg.toFixed(2)) : undefined,
      };
    });
    return data.filter(d => d.current !== undefined || d.previous !== undefined || d.baseline !== undefined); // Only show parameters with some data

  }, [allReadings, comparisonType]);


  if (!allReadings || allReadings.length === 0) {
    return <p className="text-center p-6 text-muted-foreground">No historical data for comparison.</p>;
  }
  ...
  const getPeriodRangeText = (type: string) => {
    ...
    if (type === "month") return "e.g., Current: This Month, Previous: Last Month";
    // Add other cases
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap justify-between items-center gap-4">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium">Compare period averages with:</span>
      ...
      {comparisonChartData.length > 0 ? (
        <ResponsiveContainer width="100%" height={350}>
          <BarChart data={comparisonChartData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" opacity={0.5}/>
            <XAxis dataKey="parameter" fontSize={10} interval={0} angle={-30} textAnchor="end" height={60} />
            <YAxis fontSize={10}/>
            <Tooltip 
            ...
            {comparisonType === "baseline" && (
              <Bar dataKey="baseline" name="Baseline" fill="#10b981" radius={[4, 4, 0, 0]}/>
            )}
          </BarChart>
        </ResponsiveContainer>
      ) : (
      ...
      <div className="text-xs text-muted-foreground mt-2">
        <p>* Note: "Current" refers to the ongoing period (e.g., this week/month to date). "Previous" refers to the complete prior period.</p>
        <p>* Baseline refers to an initial period of data, typically the earliest available records.</p>
