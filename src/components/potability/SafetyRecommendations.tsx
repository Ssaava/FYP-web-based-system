...
export function SafetyRecommendations({ status }: SafetyRecommendationsProps) {
    ...
    return (
      <Card>
        <CardContent className="pt-6 text-muted-foreground">
          Awaiting potability assessment...
        </CardContent>
      </Card>
    );
  }
  ...
  if (status === "safe") {
    ...
    primaryRecommendation = {
      ...
      cardClass:
        "border-yellow-200 bg-yellow-50 dark:border-yellow-600 dark:bg-yellow-900/30",
      ...
      <Card className={primaryRecommendation.cardClass}>
        <CardContent className="pt-6">
          <div className="flex items-start gap-4">
            <PrimaryIcon
              className={`h-8 w-8 ${primaryRecommendation.iconClass} mt-0.5 flex-shrink-0`}
            />
            <div>
              <h3
                className={`font-semibold text-lg ${primaryRecommendation.titleClass}`}
              >
                {primaryRecommendation.title}
              </h3>
              <p
                className={`text-sm ${primaryRecommendation.textClass} mt-1 leading-relaxed`}
              >
      ...
      <Card>
        ...
        <CardContent>
          <ul className="text-sm text-muted-foreground space-y-1.5">
            <li>
              • Always be aware of local advisories regarding water quality.
            </li>
            <li>
              • Ensure your home plumbing is well-maintained to prevent
              contamination.
            </li>
            <li>• If you use a private well, test its water regularly.</li>
            <li>
              • Report any sudden changes in water taste, odor, or appearance to
              authorities.
            </li>
            <li>
              • Store water safely in clean, covered containers if needed.
            </li>
          </ul>
        </CardContent>
      </Card>
