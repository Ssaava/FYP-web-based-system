...
import { PotabilityHistoryEntry } from "@/types";
  ...
  if (!data || data.length === 0) {
    return (
      <p className="text-center p-6 text-muted-foreground">
        No potability history available.
      </p>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Individual AI potability predictions over time (0% = Not Potable, 100%
          = Potable)
        </p>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <XAxis
              ...
              axisLine={true}
              interval={
                chartData.length > 10 ? Math.floor(chartData.length / 6) : 0
              }
            />
            <YAxis
              domain={[0, 100]}
            ...
            <Tooltip
              formatter={(value: number, name, props) => {
                const statusText =
                  props.payload.status === "safe" ? "Potable" : "Not Potable";
              ...
              {tableData.map((record) => (
                <TableRow key={record.timestamp}>
                  ...
                  <TableCell>
                    <Badge
                      variant={
                        record.status === "safe" ? "default" : "destructive"
                      }
                      className={
                        record.status === "safe"
